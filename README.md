...
- 当前数据范围：**2026-01-05 ~ 2026-09-19，共 207 场**
...
└── peak-arena-ledger/                         # 工程目录（原 jitan/）
    ...
    │   ├── jd_records.json                    # 解析出的 459 条原始记录
...
fetch_all.py      逐篇抓正文 → data/articles/<id>.txt
   ↓
extract_jd.py     双策略解析「巅峰赛场」→ data/jd_records.json（459 条原始记录）
   ↓
build_output.py   按 2026 起算 + 去重 → output/巅峰赛场战绩_2026.csv（207 场）
   ↓
daily_update.py   同步根目录交付物 CSV / HTML
   ↓
xlsx-builder/build.py  生成 华府卫视巅峰赛场战绩_2026.xlsx
```

### 解析器要点（`extract_jd.py`）

- **章节定位**：`【\s*华府(?:卫视)?[^】]{0,10}战报\s*】`
  （战报段表头有三种写法：主流是 `【华府卫视昨日战报】`，偶有省略「卫视」的
  `【华府昨日战报】`，个别期次整个段落**漏写 `【】` 表头**。）
- **无表头兜底**：若正文没有匹配到战报段，且正文提到「华府」，则以编号的
  `N、巅峰赛场` 行为锚点解析该条目，避免整段被跳过。
...
- **2026-09-23 修正**：此前表头只认 `【华府卫视…战报】`，导致两期战报被整段跳过，
  漏记 3 场（`2026-03-27 JD 4:0 Bisu`、`2026-03-27 JD 1:2 Bisu`、`2026-09-19 Speed 4:1 JD`）。
  已按上述两条规则修正，全量 277 篇逐条 diff 校验：**仅新增 4 条原始记录、0 条丢失、0 条误增**。
...
- **战报段表头不统一**：源头偶尔把 `【华府卫视昨日战报】` 写成 `【华府昨日战报】`
  （2025-12-18、2026-09-20 两期），或整段漏写 `【】` 表头（2026-03-28）。
  解析器已按「表头两种写法 + 编号锚点兜底」处理，新增期次若再出现新写法需同步扩展。
...
→ 生成 CSV/MD/HTML → 同步根目录交付物（CSV / HTML）→ 生成 xlsx → 覆盖上传飞书 → 提交并推送 GitHub。
